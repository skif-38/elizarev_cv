...
while True:
    ...
    if len(target_colors) < num_balls:
        cv2.putText(frame, f"Kolihestvo sharov  {num_balls}/  {len(target_colors)}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (225, 255, 0))
        if clicked:
            clicked = False
    ...
    if len(shuffled_colors) == num_balls:
        ...
        if flag == num_balls:
            cv2.putText(frame, "Otgadal!", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0))
        else:
            cv2.putText(frame, f"Nepravilnoe raspolozhenie", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (225, 255, 0))
