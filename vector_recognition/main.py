...
def w(region, transpose=False) -> float:
    shape = region.image.shape
    image = region.image
    if transpose:
        image = image.T
...
def extractor(region):
    ...
    x_bukvi = w(region, transpose=True)
    y_bukvi = w(region)
    return np.array([region.area/region.image.size, cy, cx, perimeter, holes, v, h, eccentricity, aspect, x_bukvi, y_bukvi])
